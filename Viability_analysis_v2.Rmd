...
```{r frequencies}
    ...
    # Read counts for all the original ligation mixes
      cod.data <- read.csv(file="start_freq_codon_combined.csv")
...
```{r}
  ...
  # Summarize results in a smry table 
    ...
    smry.table.,ord <- smry.table[order(smry.table$Site),]
    setwd("/Users/craigmiller/Dropbox/protein G structure/1st Paper/jt_code/phiX_biophysical_modeling/results/") 
    write.csv(smry.table.ord, file="Posterior_probabilities_3_23_22.csv", row.names=F)
```
...
####5. Import singles, update viability matrix
```{r}
  # Import singles results
    setwd("/Users/craigmiller/Dropbox/protein G structure/1st Paper/jt_code/phiX_biophysical_modeling/") 
    singles <- read.csv(file="rawdata/singles_trimmed.csv")
    singles <- cbind(singles, mut=paste(singles$Site, singles$AA, sep=""))
    
  # Update viability matrix with singles and output it
    sites.in.viab.0 <- sapply(rownames(viab.matrix.trim), function(x) unlist(strsplit(x, "_"))[1])
    sites.in.viab <- as.numeric(sapply(sites.in.viab.0, function(x) substring(x, 2, nchar(x))))
    viab.matrix.all <- viab.matrix.trim
    rownames(viab.matrix.all) <- sites.in.viab
    
    single.viabs <- which(singles$Viable == 1)
    for (i in 1:length(single.viabs)){
      site <- singles$Site[single.viabs[i]]
      aa <- as.character(singles$AA[single.viabs[i]])
      site.row <- which(rownames(viab.matrix.all)==site)
      aa.col <- which(colnames(viab.matrix.all)==aa)
      viab.matrix.all[site.row, aa.col] <- 1
    }
    write.csv(viab.matrix.all, file="results/Viability_matrix_wsingles.csv")
    
    
```


#### 6. Make heatmap of posteriors
```{r}
    
    
  # Create plot
  setwd("/Users/craigmiller/Dropbox/protein G structure/1st Paper/jt_code/phiX_biophysical_modeling/results/") 
  png(filename="Posterior_heatmap.png", width=800, height=800)
    plot(0,0, type="n", xlim=c(0.75,25), ylim=c(-1.5,21), ylab="Amino Acid", xlab="Site", xaxt="n", yaxt="n", bty="n")
    ...
    for (site.i in 1:21){
      ...
      wt.aa <- data.wt[smry.table.trim$Site[site.i]]
      for (aa.i in 1:20){
        aa <- colnames(scale.probs.simp[aa.i])
        mut <- paste(site, aa, sep="")
        border <- NA
        ...
        } else if (mut %in% singles$mut){
            dex <- which(singles$mut == mut)
            if (length(dex)==1){
...
for (site.i in 1:21){
  ...
  rect(site.i-1,aa.dex-1, site.i, aa.dex, col=NA, border="black", lwd=2)
    ...
    lab.cex <- 1.2
    scale.cex <- 1.2
    text(24, 10.5, labels="P(viable)", cex=lab.cex)
    ...
    for (i in 1:5){
      ...
      text(22.75, i+4.5, labels=vals[i], cex=scale.cex)
    } 
    text(24, 3.5, labels="P(obs=true)", cex=lab.cex)
    ...
    for (i in 1:5){
      ...
      text(22.75, i-2.5, labels=vals2[i], cex=scale.cex)
    }  
    
    rect(23.5, 14, 24.5, 15, border="black", lwd=2)
    text(24, 15.5, labels="Wildtype", cex=lab.cex)
    
    for (i in 0:21){  #vertical lines
      points(x=c(i,i), y=c(-2,20), lwd=0.3, col="grey", type="l")
    }
    for (j in -2:20){  #horizongtal lines
      points(x=c(0,21), y=c(j,j), col="grey", type="l", lwd=0.3)
    }
